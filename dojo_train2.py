...
def _estimate_max_reward(env: PacmanEnv, alpha=0.5, c=100.0) -> float:
    pellet_count = int(env.maze.pellets.sum())
    return (max(1, pellet_count) ** (1 - alpha)) * c
...
def train_stage(
    ...
    Adds reward scaling, fast fine-tune for pretrained weights, and skip if already optimal.
    """

    alpha, c = 0.5, 100.0  # reward-scaling parameters
    ...
    if pretrained_path and os.path.exists(pretrained_path):
        ...
        eval_env.close()
        avg_eval = total_eval / 3.0
        if avg_eval >= 0.95 * max_possible:
            print(f"⚡ Already optimal ({avg_eval:.2f}/{max_possible:.2f}); skipping retrain.")
